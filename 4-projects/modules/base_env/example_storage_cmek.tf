...
module "gcs_buckets" {
  ...
  project_id              = module.floating_project.project_id
  location                = var.location_gcs
  name                    = "${var.gcs_bucket_prefix}-${module.floating_project.project_id}-cmek-encrypted-${random_string.bucket_name.result}"
  ...
  depends_on = [module.floating_project]
}
