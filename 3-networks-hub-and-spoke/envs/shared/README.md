...
| hub\_dns\_enable\_inbound\_forwarding | Toggle inbound query forwarding for Hub VPC DNS. | `bool` | `true` | no |
| hub\_dns\_enable\_logging | Toggle DNS logging for Hub VPC DNS. | `bool` | `true` | no |
| hub\_firewall\_enable\_logging | Toggle firewall logging for VPC Firewalls in Hub VPC. | `bool` | `true` | no |
| hub\_nat\_bgp\_asn | BGP ASN for first NAT cloud routes in Hub. | `number` | `64514` | no |
| hub\_nat\_enabled | Toggle creation of NAT cloud router in Hub. | `bool` | `false` | no |
| hub\_nat\_num\_addresses\_region1 | Number of external IPs to reserve for first Cloud NAT in Hub. | `number` | `2` | no |
| hub\_nat\_num\_addresses\_region2 | Number of external IPs to reserve for second Cloud NAT in Hub. | `number` | `2` | no |
| hub\_windows\_activation\_enabled | Enable Windows license activation for Windows workloads in Hub. | `bool` | `false` | no |
